...
namespace database
{
    ...
    std::ostream& operator<<(std::ostream& os, const Contact& contact)
    {
        ...
        if(contact.__affPtr)
        {
            ...
            while(printer)
            {
                os << (printer->__contact)[1] << " " << (printer->__contact)[3];
    ...
    std::istream& operator>>(std::istream &is, Contact& contact)
    {
        ...
        while(is.peek() != '|')
        {
            ...
            lastName.erase(0,1);                   //Get rid of the ' ' at the start
            std::getline(is, phone, ',');
            if(phone[0] == ' ')
              phone.erase(0,1);
            std::getline(is, email, ';');
            //One provided file has spaces between email/phone and , the other doesn't, this deals with ambiguity
            if(email[0] == ' ')
              email.erase(0,1);
