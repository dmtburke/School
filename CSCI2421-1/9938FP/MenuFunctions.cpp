...
namespace database
{
    ...
    void readMenu(TNode* &db)
    {
        ...
        menu[1] = "Use the default file name (database.txt)";
    ...
    bool dbisValid(std::string filename)
    {
        ...
        while(!fin.eof())
        {
            ...
            if(tests.length() != 9)
            {
                std::cout << "ERROR: BAD ID " << tests << " (" << current << ")" << std::endl;
                return false;
            }
            //////////////////////////////////////
            ...
            if(fin.peek() != '|' && fin.peek() != 'A')
            {
                std::cout << "ERROR: CONTACT NOT ENDED PROPERLY (" << current << ')' << std::endl;
                return false;
            }
            //////////////////////////////////////
            ...
            while(fin.peek() != '|')
            {
                ...
                if(tests != "Affiliate " + std::to_string(i))
                {
                    std::cout << "ERROR: BAD AFFILIATE (" << current << ',' << i << ')' << std::endl;
                    std::cout << "Expected: Affiliate " << i << ", Got: " << tests<< std::endl;
                    return false;
                }
                ...
                {
                    std::cout << "ERROR: AFFILIATE DELIMITER ISSUE (" << current << ',' << i << ')' << std::endl;
                    return false;
                }
                i++;
            }
            ...
            //ignore the '|', and start looking at the next contacts ID
            fin.ignore(256, '\n');

            //if we are at the end of the file we need to provoke the eof bit to make the loop end in time
            fin.peek();
