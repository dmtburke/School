...
namespace main_savitch_5
{
	...
	node(
	    const value_type& init_data = value_type( ),
	    node* init_fwd_link = NULL,
        node* init_bk_link = NULL
	)
	{ data_field = init_data; fwd_link_field = init_fwd_link; bk_link_field = init_bk_link; }

	// Member functions to set the data and link fields:
    	void set_data(const value_type& new_data) { data_field = new_data; }
    	void set_bk_link(node* new_link)             { bk_link_field = new_link; }
        void set_fwd_link(node* new_link)            { fwd_link_field = new_link; }
	...
	// the current link:
	const node* bk_link( ) const { return bk_link_field; }
    	node* bk_link( )             { return bk_link_field; }

    const node* fwd_link( ) const { return fwd_link_field; }
        node* fwd_link( )           { return fwd_link_field; }
